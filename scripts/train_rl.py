...
class RLAgentTrainer(object):
    ...
    def __init__(self):
        ...
        self.num_episodes = rospy.get_param('~num_episodes', 100)
        ...
        alpha_numerator = rospy.get_param('~agent/alpha_numerator', 1)
        ...
        self.agent = learners.EpsilonGreedyQTableAgent(
            ...
            lambda eps: alpha_start * alpha_numerator / (alpha_numerator + eps),
    ...
    def train(self):
        ...
        for eps in xrange(self.num_episodes):
            ...
            while status == Status.IN_PROGRESS:
                ...
                reward = self.task.R(state, action)
            ...
            # Completed an episode. Update pi and then reset
            # self.agent.update_pi()
            self.task.reset_task_state()
            self.agent.reset()
        ...
        learners.save_agent(self.agent, agent_filename)
