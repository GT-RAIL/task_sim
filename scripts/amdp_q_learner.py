...
from task_sim.str.amdp_qs_learned import AMDPQsLearned
from learn_q_function import LearnQFunction
...
class AMDPQLearner(object):
    ...
    def __init__(self):
        ...
        # Instantiate the transition functions and the utility functions (for higher level amdps)
        self.Ts = {}
        self.Us = {}
        # Instantiate the Q table (for leaf node amdps)
        self.Qs = {}
        ...
        # Instantiate interfaces to the environments. Q-learning takes 7 environments
        self.simulators = {} # Format: (amdp_id, simulator_name). None -> full task
        self.simulators[0] = rospy.get_param('~simulators/drawer_o', 'drawer0')
        self.simulators[1] = rospy.get_param('~simulators/drawer_c', 'drawer1')
        self.simulators[2] = rospy.get_param('~simulators/drawer_p', 'drawer2')
        self.simulators[6] = rospy.get_param('~simulators/box_o', 'box0')
        self.simulators[7] = rospy.get_param('~simulators/box_c', 'box1')
        ...
        for container, amdp_ids in self.demo_envs:
            for amdp_id in amdp_ids:
                ...
                self.q_learners[(container, amdp_id,)] = \
                    LearnQFunction(
                        amdp_id, container,
        ...
        self.amdp_node = AMDPNode(self.simulators[None], self.Ts, self.Us, self.demo_mode, self.baseline_mode, True, self.Qs)

    def _run_q_learner(self, learner, epoch):
    ...
    def train(self, epochs=2502, test_every=10, save_every=100):
        ...
        while epoch < epochs:
            ...
            learn_workers = []
            for key, q_learner in self.q_learners.iteritems():
                ...
                worker = threading.Thread(
                    target=self._run_q_learner,
                    args=(q_learner, epoch,)
                )
                learn_workers.append((key, q_learner, worker,))
                worker.start()

            for key, q_learner, worker in learn_workers:
                worker.join()
                print(
                    "Trained:", key,
                    "Epoch:", q_learner.epoch,
                    "\tSuccesses:", q_learner.successes,
                    "\tAction executions:", q_learner.action_executions
                )
            ...
            if epoch % test_every == 0 and epoch > 0:
                # TODO: Need better state management for speedup with multiprocessing here
                ...
                ex_count = 0
                for key, q_learner in self.q_learners.iteritems():
                    ex_count += q_learner.action_executions
                rate_demo = success_rate_demo/(10*eval_trials)
                rate_train = success_rate_train/((len(self.task_envs) - 10)*eval_trials)
                rate_combined_train = (success_rate_demo + success_rate_train)/(len(self.task_envs)*eval_trials)
                rate_test = success_rate_test/(len(self.test_envs))
                print("Epoch:", epoch,
                      ...
                      "\tAction executions:", ex_count)
                for key, q_learner in self.q_learners.iteritems():
                    print('\t', key, q_learner.action_executions, 'training action executions')
                print("\n**********************************************************************************")
